...
- **Replace territory** — the recommended solution conflicts with current code, the architectural approach changed, or the pattern is no longer the preferred way. This means a new learning needs to be written. A replacement writer creates the successor following `ce-compound`'s document format (frontmatter, problem, root cause, solution, prevention), using the investigation evidence already gathered. Use a replacement subagent for context isolation when available and permitted; otherwise the orchestrator may write a small, well-evidenced replacement in the main thread, or mark the old learning stale when the replacement would be too broad or uncertain.
...
In Codex, use subagents only when multi-agent delegation is available and permitted in the current harness. If subagents are unavailable or not permitted, keep investigation in the main thread. For Replace candidates, write the replacement in the main thread only when the evidence is sufficient and the successor is small enough to produce confidently; otherwise mark the old learning stale and report what evidence is missing. In headless mode, prefer stale-marking over main-thread replacement when the scope is not clearly bounded.
...
|---------|---------------|
| On main/master | Create a branch named for what was refreshed (e.g., `docs/refresh-auth-and-ci-learnings`) and commit. Do not open a PR. |
...
1. Create a branch and commit (recommended) — the branch name should be specific to what was refreshed, not generic (e.g., `docs/refresh-auth-learnings` not `docs/compound-refresh`). Do not open a PR.
...
- Considers the repo's existing commit conventions (check recent git log for style), but does not force a specific message format
