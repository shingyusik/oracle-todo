...
| **Headless** | `mode:headless` in arguments | No blocking questions. Run **Full mode without session history** when subagents are available and permitted; otherwise run Lightweight mode. Apply the Discoverability Check edit silently if a gap exists. Skip Phase 3 specialized reviews. End with a structured terminal report — no "What's next?" menu. |

Headless mode is intended for automations and skill-to-skill invocation where no human is present to answer questions. When Full mode is available, the doc is identical to what an interactive Full run would produce — classification work (track, category, overlap) follows the same rules and writes nothing extra into the artifact. When Full mode is unavailable, headless falls back to Lightweight mode rather than blocking. Once detected, headless mode applies for the entire run.
...
**In headless mode**, skip both questions below and go directly to **Full Mode** with session history disabled when subagents are available and permitted; otherwise run Lightweight Mode. Phase 1's session-history step (step 4) is omitted. Proceed straight to research only when Full Mode is available.

In Codex, Full mode may use subagents only when multi-agent delegation is available and permitted in the current harness. If subagents are unavailable or not permitted, run Lightweight Mode instead of blocking. When `ce-compound` is invoked automatically from a completion checkpoint rather than by an explicit user request, prefer Lightweight Mode unless the user also requested multi-agent, parallel, or delegated work.
...
#### 3. **Related Docs Finder**
   ...
   - If `docs/solutions/` does not exist yet, return no related docs, no refresh candidates, and no overlap. Do not treat the missing directory as a workflow failure; the orchestrator creates the needed category directory in Phase 2.
...
#### 4. **Session History via `ce-sessions`** (synchronous skill call, after launching the parallel block — only if the user opted in)
   ...
   - Invoke `ce-sessions` through the current platform's skill-loading mechanism. In Codex, expand the available skill path for `ce-sessions`, read its `SKILL.md`, and follow it in the main context; there is no separate `Skill` tool. Pass the dispatch payload below as the skill argument string. `ce-sessions` runs in main context — it owns discovery, branch/keyword filtering, scan-window selection, the deep-dive cap, per-session extraction to a `mktemp` scratch dir, and dispatch of the synthesis-only `ce-session-historian` subagent. The compound orchestrator only needs to pass the topic and time window and read back the findings text.
...
8. **Run the bundled frontmatter validator relative to this `SKILL.md`: `python3 <ce-compound-skill-dir>/scripts/validate-frontmatter.py <output-path>`** to catch silent-corruption parser-safety issues that the prose rules miss. In this repository, the path is `python3 .codex/skills/ce-compound/scripts/validate-frontmatter.py <output-path>`. It catches malformed `---` delimiter lines, unquoted ` #` in scalar values (silent comment truncation), and unquoted `: ` in scalar values (silent mapping confusion). Exit 0 means the doc is parser-safe; exit 1 means the script's stderr names the offending field(s) and what to fix — quote the value(s), re-write the doc, and re-run until exit 0. Do not declare success while validation fails. The script does not enforce schema rules and does not flag YAML reserved-indicator characters (those produce loud parser errors downstream rather than silent corruption — out of scope). Uses Python 3 stdlib only (no PyYAML or other deps).
...
Headless mode uses Full when subagents are available and permitted; otherwise it may enter Lightweight so automations can complete without blocking questions.
</critical_requirement>
...
When Lightweight Mode is used as a headless fallback, run the Discoverability Check after writing the learning and apply the instruction-file edit silently if a gap exists. When Lightweight Mode is interactive, do not block for discoverability consent; output the one-line tip described below and move on.
