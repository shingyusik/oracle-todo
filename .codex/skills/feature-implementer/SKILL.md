...
description: "Creates phase-based feature implementation plans with quality gates, test-first workflow, and incremental delivery structure. Default to Clean Architecture unless the user explicitly requests another architecture. Use when implementing features, planning execution phases, breaking down work, or structuring delivery strategy. Keywords include feature implementation, implement, planning, phases, breakdown, clean architecture, strategy, and roadmap."
---
...
- Use Clean Architecture by default unless user explicitly requests another architecture

## Planning Workflow

### Default Architecture Policy
1. Default to Clean Architecture for every feature plan.
2. Apply this default unless user explicitly specifies a different architecture.
3. Map each task to Clean Architecture layers:
   - Domain layer for entities and business rules
   - Application layer for use cases and orchestration
   - Interface adapters for controllers, presenters, and repository interfaces
   - Infrastructure layer for frameworks, databases, and external services
4. Enforce dependency rule: outer layers depend on inner layers, inner layers never depend on outer layers.
...
5. Check whether user provided architecture override; if not, lock plan to Clean Architecture.
...
- Defines clear layer boundaries aligned to Clean Architecture
...
- **Architecture Mapping**: Domain/Application/Adapters/Infrastructure responsibilities for this phase

### Step 3: Plan Document Creation
Use plan-template.md to generate a plan document in the project's planning-doc path.
Default path: `docs/plans/PLAN_<feature-name>.md`
If the repository already has an equivalent planning path (for example `plans/`, `docs/roadmap/`, `docs/implementation/`), create the plan there instead of forcing `docs/plans/`.
...
1. Determine the planning-doc directory:
   - Prefer an existing project path used for plan/roadmap/implementation documents
   - Fall back to `docs/plans/` when no equivalent path exists
2. Create the chosen planning directory if not exists
...
**Architecture**:
- [ ] Clean Architecture is used unless user explicitly requested otherwise
- [ ] Dependency rule is respected (inner layers do not depend on outer layers)
