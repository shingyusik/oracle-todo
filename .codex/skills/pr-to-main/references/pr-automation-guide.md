...
---
## Step 0 - Branch guard and confirmation

Validate branch before any PR work. If current branch is not `develop`,
stop immediately and tell the user to rerun from `develop`.

```bash
CURRENT="$(git branch --show-current)"
if [ "$CURRENT" != "develop" ]; then
  echo "현재 브랜치가 develop이 아닙니다. develop 브랜치에서 다시 스킬을 실행해주세요."
  exit 1
fi

echo "[PR] ${CURRENT} → main"
```
...
PR_NUMBER="$(gh pr list --base main --head "$CURRENT" --state open \
