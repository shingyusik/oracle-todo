...
Run the discovery + metadata pipeline with scripts resolved relative to this `SKILL.md` (preserving the null-delimited xargs hardening that lets `extract-metadata.py` run in batch mode). In this repository, `<ce-sessions-skill-dir>` is `.codex/skills/ce-sessions`.

```bash
bash <ce-sessions-skill-dir>/scripts/discover-sessions.sh <repo> <days> | tr '\n' '\0' | xargs -0 python3 <ce-sessions-skill-dir>/scripts/extract-metadata.py --cwd-filter <repo>
```
...
2. **If the branch filter returned zero sessions, or you're processing Codex sessions:**
   ...
   - Re-invoke the discovery pipeline with `--keyword K1,K2,...` appended to the `<ce-sessions-skill-dir>/scripts/extract-metadata.py` invocation. The script returns sessions with non-zero `match_count` plus per-keyword counts.
...
```bash
python3 <ce-sessions-skill-dir>/scripts/extract-skeleton.py --output "$SCRATCH/<session-id>.skeleton.txt" < <session-file>
```
...
```bash
python3 <ce-sessions-skill-dir>/scripts/extract-skeleton.py --output "$SCRATCH/<session-id>.skeleton.tail.txt" < <session-file>
```
...
```bash
python3 <ce-sessions-skill-dir>/scripts/extract-errors.py --output "$SCRATCH/<session-id>.errors.txt" < <session-file>
```
...
In Codex, use the synthesis subagent only when multi-agent delegation is available and permitted in the current harness. If it is unavailable or not permitted, synthesize in the main context from the extracted scratch files instead of blocking. Keep the same deep-dive cap, read only the extracted skeleton/error files (never the original session files), and keep the output schema unchanged. If the extracted files are too large to read comfortably, inspect their sizes and read focused head/tail or keyword snippets from the scratch files, then note the partial synthesis in the output.
